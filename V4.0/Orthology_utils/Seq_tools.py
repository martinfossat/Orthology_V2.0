...
class Seq_Prop_Manager:
    ...
    def get_values(self,seq_id,label,bounds_label,val_type):
        if not seq_id in self.SeqProp:
            return float('NaN')
        elif not label in self.SeqProp[seq_id]:
            return float('NaN')
        elif not bounds_label in self.SeqProp[seq_id][label]:
            return float('NaN')
        elif not val_type in self.SeqProp[seq_id][label][bounds_label]:
            return float('NaN')
        else :
...
def get_kappa(seq,window_size=5):
    ...
    d=get_delta(seq_q,window_size)
    # The definition of dmax is not just sorting.
...
def apply_mask(database,mask):

    out=copy.deepcopy(database)
    for prop in database:
        for orga in database[prop]:

            tmp=np.array(database[prop][orga])[np.array(mask[orga])]
            out[prop][orga]=tmp[np.invert(np.isnan(tmp))]
