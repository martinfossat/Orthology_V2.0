...
SCRIPT_LOC='/cn1/scratch/Git/Orthology_V2.0/V3.0'
...
for((i=${batch_start};i<${N_batches};i++));
do
...
echo python ${SCRIPT_LOC}/1_Get_orthologs_V1.3.py -f ./Batches/Input_names/batch_${i}.txt -of ${ortho_file} -sf ${seq_file} -nf ${names_file} -os drerio -as hsapiens mmusculus>> run.batch
echo python ${SCRIPT_LOC}/2_Get_IDRs_V2.0.py -sf ${seq_file} -pf ${seq_prop_file} >> run.batch
echo python ${SCRIPT_LOC}/3_Add_sequence_properties_V1.1.py -pf ${seq_prop_file} -sf ${seq_file} -of ${ortho_file}  >> run.batch
echo python ${SCRIPT_LOC}/4_Compare_sequences_V1.3.py -of ${ortho_file} -hf ${homo_file} -sf ${seq_file} -pf ${seq_prop_file} -rs hsapiens -as drerio mmusculus -mlf 0.4 >> run.batch
